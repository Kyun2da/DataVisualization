...
d3.csv("/accident.csv", function(d) {
  for (var i = 0; i < 2557; i++)
    if (d["발생년월일시분"] == dataset_4[i].Date) {
      dataset_4[i].사고수 += 1;
      dataset_4[i].사상자수 += parseInt(d["사상자수"]);
      dataset_4[i].사망자수 += parseInt(d["사망자수"]);
      break;
    }
});
//2012~2014년 데이터 파싱
...
setTimeout(() => {
  ...
  var simulation = d3
    .forceSimulation()
    .force("x", d3.forceX(w_2 / 2).strength(0.05))
    .force("y", d3.forceY(h_2 / 2).strength(0.05))
    .force(
      "collide",
  ...
  simulation.nodes(dataset_4).on("tick", ticked);
  function ticked() {
    circles
      .attr("cx", function(d) {
        return d.x;
